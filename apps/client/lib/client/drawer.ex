defmodule Drawer do
  ...
  defp _fill(left, middle, right, fill) do
    "#{left}#{String.pad_trailing("", 19, fill)}#{middle}#{String.pad_trailing("", 20, fill)}#{right}\n"
  end
  ...
  defp _parse_entities(players, enemies, player_id, turn_id) do
    _fill("┌", "┬", "┐","─") <>
    "│#{String.pad_trailing("JUGADORES", 19)}│ #{String.pad_trailing("ENEMIGOS", 19)}│\n" <>
    _fill("├", "┼", "┤","─") <>
    List.to_string(for i <- 0..max(length(players), length(enemies)) do
      ...
      parsed_e = _format_color(:red, _color_turn(turn_id, e))
